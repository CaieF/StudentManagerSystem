...
router.post('/', function(req, res, next) {
  ...
  form.parse(req,function(err,fields,files){
    console.log(fields);
    let updateTcid = fields.updateTcid[0];
          ...
          if(teacherData.length > 0){
            db.query('SELECT COUNT(*) as count FROM teacher_course WHERE tno = ? AND cno = ?',[teacherData[0].tno,courseData[0].cno],function(err,existingData){
              ...
              if(existingData[0].count == 0){
                db.query(`update teacher_course set tno=?,cno=? where tcid=${updateTcid}`,[teacherData[0].tno,courseData[0].cno],function(err,data){
                  if(err){
                    throw err;
                  }else{
                    //更新
