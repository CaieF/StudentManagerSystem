...
router.get('/', function(req, res, next) {
  ...
  //拿到的搜索value数据
  let like = req.query.likeStuGPA || req.query.selectVal;
    db.query(`SELECT * FROM student_gpa_view WHERE  class LIKE '%${like}%' OR name LIKE '%${like}%'`,function(err,data){
      let pager = {};
      //当前第几页，默认1
      ...
      if(err){
        ...
        db.query('SELECT DISTINCT s.class FROM student s ORDER BY s.class',function(err,Class){
          if(err){
            ...
            res.render('studentGPA',{studentGPA:dataList,pager,Class,user});
          }
        })
      }
    })
