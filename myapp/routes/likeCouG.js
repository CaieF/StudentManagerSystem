...
router.get('/', function(req, res, next) {
  ...
  //拿到的搜索value数据
  let like = req.query.likeCouG || req.query.selectVal;
  db.query(`SELECT * FROM course_grade_view WHERE  name LIKE '%${like}%'`,function(err,data){
    let pager = {};
    //当前第几页，默认1
    ...
    if(err){
      ...
      db.query('SELECT name FROM course',function(err,Course){
        if(err){
          ...
          res.render('courseGrade',{courseGrade:dataList,Course,pager,user});
        }
      })
