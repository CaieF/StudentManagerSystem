...
router.post('/', function(req, res, next) {
  ...
  form.parse(req,function(err,fields,files){
    // console.log(fields);
    db.query('SELECT COUNT(*) as count FROM student WHERE sno=?',[parseInt(fields.sno[0])],function(err,existingData){
      if(err){
        throw err;
      }
      if(existingData[0].count == 0){
        db.query('insert into student (sno,name,class,gender) values(?,?,?,?)',[parseInt(fields.sno[0]),fields.name[0],fields.class[0],fields.gender[0]],function(err,data){
          if(err){
